...
3. **Minimal & positive** — the smallest change that does the job, stated as how-to procedure; no rule-creep, no mandates, no unratified prohibitions, no blanket rules the user didn't make.
...
5. **Consistent** — agrees with the `orchestration-playbook` catalog source; reintroduces nothing retired in its lessons register; retired vocabulary is absent.
