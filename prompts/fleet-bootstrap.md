...
When the user describes a goal or project: read `playbook/SKILL.md` and either assume the orchestrator role yourself or launch a better-suited agent to hold it, per the playbook. Restate the goal with a finish line — "Done means: X" — before starting. Work lands in git; on repos whose main is locked, changes go by pull request. Record what happens per the documentation contract (`playbook/documentation.md`) — documentation is part of Done, and the notebook is the asset; tools are just tools.
